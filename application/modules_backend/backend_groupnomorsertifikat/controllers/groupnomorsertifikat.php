...
class Groupnomorsertifikat extends MY_Admin {
	private $_template 			= 'template_admin/main';
	private $_module_controller = 'backend_groupnomorsertifikat/groupnomorsertifikat/';
	private $_table_name 		= 'groupnomorsertifikat';
	private $_table_field_pref 	= 'grno_';
	private $_table_pk 			= 'grno_id';
	private $_model_crud 		= 'crud_model';

	private $_page_title 		= 'Layanan : Group Nomor Sertifikat List';
	private $_page_content_info	= array(
		'title' => 'Data Group Nomor Sertifikat',
		'desc' 	=> 'List Group Nomor Sertifikat',
	);
	...
	private function get_show_column() {
		$column_list = array(
			...
			array(
				'title_header_column' 	=> 'Nomor Sertifikat',
				'field_name' 			=> $this->_table_field_pref . 'nomor',
				'no_order'				=> 2,
			),
			array(
				'title_header_column' 	=> 'Nomor Urut',
				'field_name' 			=> $this->_table_field_pref . 'nourut',
				'no_order'				=> 3,
			),
			array(
	...
	private function get_input_field_new($data_value = array()) {
		$data_input = array(
			...
			array(
				...
				'db_process'	=> true,
				'input_type'	=> 'text',
				'input_attr'	=> 'type="text" data-parsley-minlength="1" class="form-control" placeholder="Nomor Sertifikat..."',
				'required'		=> 'required',
				'data_source'	=> '',
				'data_edit'		=> array(
					'db_process'	=> true,
			...
			array(
				...
				'db_field' 		=> $this->_table_field_pref . 'nomor',
				'db_process'	=> true,
				'input_type'	=> 'text',
				'input_attr'	=> 'type="text" data-parsley-minlength="1" class="form-control" placeholder="Nomor Sertifikat..."',
				'required'		=> 'required',
			...
			array(
				'label' 		=> 'Nomor Urut',
				'db_field' 		=> $this->_table_field_pref . 'nourut',
				'db_process'	=> true,
				'input_type'	=> 'text',
				'input_attr'	=> 'type="text" data-parsley-minlength="1" class="form-control" placeholder="Nomor Urut..."',
				'data_source'	=> '',
				'data_edit'		=> array(
					'db_process'	=> true,
				),
	...
	function do_add() {
		...
		if($this->form_validation->run()) {
			$res = false;
			if(!empty($admin_data)) {
				$admin_data['grno_entryuser'] = $this->session->userdata('admin_username');
				$admin_data['grno_entrydate'] = date('Y-m-d H:i:s');
				$res = $this->crudmodel->posts_groupnomorsertifikat($admin_data);
				if($res) {
	...
	function edit_ajax() {
		...
		$data = $this->crudmodel->where(array($this->_table_pk => $data_id ))->get_row_groupnomorsertifikat();
	...
	function do_edit() {
		...
		if($this->form_validation->run()) {
			...
			if(!empty($admin_data)) {
				$admin_data['grno_changeuser'] = $this->session->userdata('admin_username');
				$admin_data['grno_changedate'] = date('Y-m-d H:i:s');
				$res = $this->crudmodel->where(array($field_pk => $user_id))->puts_groupnomorsertifikat($admin_data);
				if($res) {
	...
	function do_delete() {
		if($this->input->post('data_id')) {
			$delete = $this->crudmodel->delete_groupnomorsertifikat(array($this->_table_pk => $this->input->post('data_id')));
			if($delete) {
