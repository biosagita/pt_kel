...
class Templatesertifikat extends MY_Admin {
	private $_template 			= 'template_admin/main';
	private $_module_controller = 'backend_templatesertifikat/templatesertifikat/';
	private $_table_name 		= 'templatesertifikat';
	private $_table_field_pref 	= 'tpsr_';
	private $_table_pk 			= 'tpsr_id';
	private $_model_crud 		= 'crud_model';

	private $_page_title 		= 'Layanan : Template Sertifikat List';
	private $_page_content_info	= array(
		'title' => 'Data Template Sertifikat',
		'desc' 	=> 'List Template Sertifikat',
	);
	...
	private function get_show_column() {
		$column_list = array(
			...
			array(
				'title_header_column' 	=> 'Nama Sertifikat',
	...
	private function get_input_field_new($data_value = array()) {
		$data_input = array(
			...
			array(
				...
				'db_process'	=> true,
				'input_type'	=> 'text',
				'input_attr'	=> 'type="text" data-parsley-minlength="1" class="form-control" placeholder="Nama Sertifikat..."',
				'required'		=> 'required',
				'data_source'	=> '',
				'data_edit'		=> array(
					'db_process'	=> true,
			...
			array(
				...
				'db_field' 		=> $this->_table_field_pref . 'template',
				'db_process'	=> true,
				'input_type'	=> 'textarea',
				'input_attr'	=> 'class="form-control ckeditor" placeholder="Sertifikat..."',
				'data_source'	=> '',
				'data_edit'		=> array(
					'db_process'		=> true,
					'required'		=> 'required',
	...
	function do_add() {
		...
		if($this->form_validation->run()) {
			$res = false;
			if(!empty($admin_data)) {
				$admin_data['tpsr_entryuser'] = $this->session->userdata('admin_username');
				$admin_data['tpsr_entrydate'] = date('Y-m-d H:i:s');
				$res = $this->crudmodel->posts_templatesertifikat($admin_data);
				if($res) {
	...
	function edit_ajax() {
		...
		$data = $this->crudmodel->where(array($this->_table_pk => $data_id ))->get_row_templatesertifikat();
	...
	function do_edit() {
		...
		if($this->form_validation->run()) {
			...
			if(!empty($admin_data)) {
				$admin_data['tpsr_changeuser'] = $this->session->userdata('admin_username');
				$admin_data['tpsr_changedate'] = date('Y-m-d H:i:s');
				$res = $this->crudmodel->where(array($field_pk => $user_id))->puts_templatesertifikat($admin_data);
				if($res) {
	...
	function do_delete() {
		if($this->input->post('data_id')) {
			$delete = $this->crudmodel->delete_templatesertifikat(array($this->_table_pk => $this->input->post('data_id')));
			if($delete) {
