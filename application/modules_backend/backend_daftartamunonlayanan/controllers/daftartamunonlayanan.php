...
class daftartamunonlayanan extends MY_Admin {
	private $_template 			= 'template_admin/main';
	private $_module_controller = 'backend_daftartamunonlayanan/daftartamunonlayanan/';
	private $_table_name 		= 'daftartamunonlayanan';
	private $_table_field_pref 	= 'dfnl_';
	private $_table_pk 			= 'dfnl_id';
	private $_model_crud 		= 'crud_model';

	private $_page_title 		= 'Layanan : Daftar Tamu Non Layanan List';
	private $_page_content_info	= array(
		'title' => 'Data Daftar Tamu Non Layanan',
		'desc' 	=> 'List Daftar Tamu Non Layanan',
	);
	...
	private function get_show_column() {
		$column_list = array(
			...
			array(
				'title_header_column' 	=> 'Instansi',
				'field_name' 			=> $this->_table_field_pref . 'instansi',
				'no_order'				=> 3,
			),
			array(
	...
	function do_add() {
		...
		if($this->form_validation->run()) {
			$res = false;
			if(!empty($admin_data)) {
				$admin_data['dfnl_entryuser'] = $this->session->userdata('admin_username');
				$admin_data['dfnl_entrydate'] = date('Y-m-d H:i:s');
				$res = $this->crudmodel->posts_daftartamunonlayanan($admin_data);
				if($res) {
	...
	function edit_ajax() {
		...
		$data = $this->crudmodel->where(array($this->_table_pk => $data_id ))->get_row_daftartamunonlayanan();
	...
	function do_edit() {
		...
		if($this->form_validation->run()) {
			...
			if(!empty($admin_data)) {
				$admin_data['dfnl_changeuser'] = $this->session->userdata('admin_username');
				$admin_data['dfnl_changedate'] = date('Y-m-d H:i:s');
				$res = $this->crudmodel->where(array($field_pk => $user_id))->puts_daftartamunonlayanan($admin_data);
				if($res) {
	...
	function do_delete() {
		if($this->input->post('data_id')) {
			$delete = $this->crudmodel->delete_daftartamunonlayanan(array($this->_table_pk => $this->input->post('data_id')));
			if($delete) {
