...
class Berkaslayanan extends MY_Admin {
	private $_template 			= 'template_admin/main';
	private $_module_controller = 'backend_berkaslayanan/berkaslayanan/';
	private $_table_name 		= 'berkaslayanan';
	private $_table_field_pref 	= 'bly_';
	private $_table_pk 			= 'bly_id';
	private $_model_crud 		= 'crud_model';

	private $_page_title 		= 'berkaslayanan : List Berkas Layanan';
	private $_page_content_info	= array(
		'title' => 'Data Berkas Layanan',
		'desc' 	=> 'List Berkas Layanan',
	);
	...
	private function get_show_column() {
		$column_list = array(
			...
			array(
				'title_header_column' 	=> 'Nama Pemohon',
				'field_name' 			=> $this->_table_field_pref . 'pemohon',
				'no_order'				=> 1,
			),
			array(
				'title_header_column' 	=> 'No Berkas',
				'field_name' 			=> $this->_table_field_pref . 'noberkas',
				'no_order'				=> 2,
			),
			array(
			...
			array(
				'title_header_column' 	=> 'Entry Date',
	...
	private function get_input_field_new($data_value = array()) {
		...
		$data_input = array(
			...
			array(
				'label' 		=> 'Nama Pemohon',
				'db_field' 		=> $this->_table_field_pref . 'pemohon',
				'db_process'	=> true,
				'input_type'	=> 'text',
				'input_attr'	=> 'type="text" data-parsley-minlength="1" class="form-control" placeholder="Nama berkaslayanan..."',
				'required'		=> 'required',
				'data_source'	=> '',
				'data_unique'	=> true,
				'data_edit'		=> array(
			...
			array(
				'label' 		=> 'No Berkas',
				'db_field' 		=> $this->_table_field_pref . 'noberkas',
				'db_process'	=> true,
				'input_type'	=> 'text',
				'input_attr'	=> 'type="text" data-parsley-minlength="1" class="form-control" placeholder="Nama berkaslayanan..."',
				'required'		=> 'required',
	...
	function lists_ajax() {
		...
		$table .= ' LEFT JOIN ' . $this->db->dbprefix . 'layanan ON (bly_lyn_id = lyn_id)';
	...
	function do_add() {
		...
		if($this->form_validation->run()) {
			$res = false;
			if(!empty($admin_data)) {
				$admin_data['bly_entryuser'] = $this->session->userdata('admin_username');
				$admin_data['bly_entrydate'] = date('Y-m-d H:i:s');
				$res = $this->crudmodel->posts_berkaslayanan($admin_data);
				if($res) {
	...
	function edit_ajax() {
		...
		$data = $this->crudmodel->where(array($this->_table_pk => $data_id ))->get_row_berkaslayanan();
	...
	function do_edit() {
		...
		if($this->form_validation->run()) {
			...
			if(!empty($admin_data)) {
				$admin_data['bly_changeuser'] = $this->session->userdata('admin_username');
				$admin_data['bly_changedate'] = date('Y-m-d H:i:s');
				$res = $this->crudmodel->where(array($field_pk => $user_id))->puts_berkaslayanan($admin_data);
				if($res) {
	...
	function do_delete() {
		if($this->input->post('data_id')) {
			$delete = $this->crudmodel->delete(array($this->_table_pk => $this->input->post_berkaslayanan('data_id')));
			if($delete) {
