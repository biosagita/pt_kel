...
class Persyaratanlayanan extends MY_Admin {
	private $_template 			= 'template_admin/main';
	private $_module_controller = 'backend_persyaratanlayanan/persyaratanlayanan/';
	private $_table_name 		= 'persyaratanlayanan';
	private $_table_field_pref 	= 'ply_';
	private $_table_pk 			= 'ply_id';
	private $_model_crud 		= 'crud_model';

	private $_page_title 		= 'Layanan : List Persyaratan Layanan';
	private $_page_content_info	= array(
		'title' => 'Data Persyaratan Layanan',
		'desc' 	=> 'List Persyaratan Layanan',
	);
	...
	private function get_show_column() {
		$column_list = array(
			...
			array(
				'title_header_column' 	=> 'Persyaratan',
				'field_name' 			=> $this->_table_field_pref . 'desc',
				'no_order'				=> 1,
			),
			array(
			...
			array(
				...
				'field_name' 			=> $this->_table_field_pref . 'type_persyaratan',
				'result_format'			=> function( $d, $row ) {

						switch ($d) {
							case TYPE_PERSYARATAN_BADANHUKUM:
								$d = 'Badan Hukum';
								break;
							case TYPE_PERSYARATAN_DIKUASAKAN:
								$d = 'Dikuasakan';
								break;
							case TYPE_PERSYARATAN_PROPOSALTEKNIS:
								$d = 'Proposal Teknis';
								break;
							case TYPE_PERSYARATAN_TANAHBANGUNANSEWA:
								$d = 'Tanah atau Bangunan Sewa';
								break;
							default:
								$d = '-';
								break;
	...
	private function get_input_field_new($data_value = array()) {
		$source_layanan = $this->crudmodel->get_option_layanan();
		$source_type_persyaratan = array(
			array(
				'name' => 'Badan Hukum',
				'value' => TYPE_PERSYARATAN_BADANHUKUM,
			),
			array(
				'name' => 'Dikuasakan',
				'value' => TYPE_PERSYARATAN_DIKUASAKAN,
			),
			array(
				'name' => 'Proposal Teknis',
				'value' => TYPE_PERSYARATAN_PROPOSALTEKNIS,
			),
			array(
				'name' => 'Tanah atau Bangunan Sewa',
				'value' => TYPE_PERSYARATAN_TANAHBANGUNANSEWA,
			),
		);
		...
		$data_input = array(
			...
			array(
				'label' 		=> 'Persyaratan',
				'db_field' 		=> $this->_table_field_pref . 'desc',
				'db_process'	=> true,
				'input_type'	=> 'text',
				'input_attr'	=> 'type="text" data-parsley-minlength="1" class="form-control" placeholder="Nama persyaratanlayanan..."',
				'required'		=> 'required',
				'data_source'	=> '',
				'data_edit'		=> array(
					'db_process'		=> true,
			...
			array(
				'label' 		=> 'Nama Layanan',
				'db_field' 		=> $this->_table_field_pref . 'lyn_id',
				'db_process'	=> true,
				'input_type'	=> 'select',
			...
			array(
				...
				'db_field' 		=> $this->_table_field_pref . 'type_persyaratan',
				'db_process'	=> true,
				'input_type'	=> 'select',
				'input_attr'	=> 'class="form-control"',
				'data_source'	=> $source_type_persyaratan,
				'data_edit'		=> array(
					'db_process'		=> true,
				),
			),
	...
	function lists_ajax() {
		...
		$table .= ' LEFT JOIN ' . $this->db->dbprefix . 'layanan ON (ply_lyn_id = lyn_id)';
	...
	function do_add() {
		...
		if($this->form_validation->run()) {
			$res = false;
			if(!empty($admin_data)) {
				$admin_data['ply_entryuser'] = $this->session->userdata('admin_username');
				$admin_data['ply_entrydate'] = date('Y-m-d H:i:s');
				$res = $this->crudmodel->posts_persyaratanlayanan($admin_data);
				if($res) {
	...
	function edit_ajax() {
		...
		$data = $this->crudmodel->where(array($this->_table_pk => $data_id ))->get_row_persyaratanlayanan();
	...
	function do_edit() {
		...
		if($this->form_validation->run()) {
			...
			if(!empty($admin_data)) {
				$admin_data['ply_changeuser'] = $this->session->userdata('admin_username');
				$admin_data['ply_changedate'] = date('Y-m-d H:i:s');
				$res = $this->crudmodel->where(array($field_pk => $user_id))->puts_persyaratanlayanan($admin_data);
				if($res) {
	...
	function do_delete() {
		if($this->input->post('data_id')) {
			$delete = $this->crudmodel->delete(array($this->_table_pk => $this->input->post_persyaratanlayanan('data_id')));
			if($delete) {
