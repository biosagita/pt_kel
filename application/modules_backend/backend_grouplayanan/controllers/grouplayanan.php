...
class Grouplayanan extends MY_Admin {
	private $_template 			= 'template_admin/main';
	private $_module_controller = 'backend_grouplayanan/grouplayanan/';
	private $_table_name 		= 'grouplayanan';
	private $_table_field_pref 	= 'gly_';
	private $_table_pk 			= 'gly_id';
	private $_model_crud 		= 'crud_model';

	private $_page_title 		= 'Layanan : Group Layanan List';
	private $_page_content_info	= array(
		'title' => 'Data Group Layanan',
		'desc' 	=> 'List Group Layanan',
	);
	...
	private function get_input_field_new($data_value = array()) {
		$data_input = array(
			...
			array(
				...
				'db_process'	=> true,
				'input_type'	=> 'text',
				'input_attr'	=> 'type="text" data-parsley-minlength="1" class="form-control" placeholder="Level Name..."',
				'required'		=> 'required',
				'data_source'	=> '',
				'data_edit'		=> array(
					'db_process'	=> true,
	...
	function do_add() {
		...
		if($this->form_validation->run()) {
			$res = false;
			if(!empty($admin_data)) {
				$admin_data['gly_entryuser'] = $this->session->userdata('admin_username');
				$admin_data['gly_entrydate'] = date('Y-m-d H:i:s');
				$res = $this->crudmodel->posts_grouplayanan($admin_data);
				if($res) {
	...
	function edit_ajax() {
		...
		$data = $this->crudmodel->where(array($this->_table_pk => $data_id ))->get_row_grouplayanan();
	...
	function do_edit() {
		...
		if($this->form_validation->run()) {
			...
			if(!empty($admin_data)) {
				$admin_data['gly_changeuser'] = $this->session->userdata('admin_username');
				$admin_data['gly_changedate'] = date('Y-m-d H:i:s');
				$res = $this->crudmodel->where(array($field_pk => $user_id))->puts_grouplayanan($admin_data);
				if($res) {
	...
	function do_delete() {
		if($this->input->post('data_id')) {
			$delete = $this->crudmodel->delete_grouplayanan(array($this->_table_pk => $this->input->post('data_id')));
			if($delete) {
